...
handleSubmit = (e) => {
  ...
  let url = `http://api.giphy.com/v1/gifs/search?q=${term}&api_key=EmeuOQpz488Q5eRDKiRpv5uF8aX8R4wD&rating=g`;
    fetch(url)
      .then(res => res.json())
        .then(json =>
          this.setState({
...
render(){
  ...
  componentDidMount(){
    fetch(`http://api.giphy.com/v1/gifs/search?q=YOUR%20QUERY%20HERE&api_key=EmeuOQpz488Q5eRDKiRpv5uF8aX8R4wD&rating=g`)
      .then(res => res.json())
        .then(json =>
          this.setState({
            allGifs: json.data.slice(0,3),
